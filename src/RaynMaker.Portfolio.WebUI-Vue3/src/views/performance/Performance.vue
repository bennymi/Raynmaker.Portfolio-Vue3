  ...
  <div>
    ...
    <CCard :style="{ 'margin-top': '2rem' }">
      ...
      <CCardBody v-if="benchmark">
        <p>
          If you would have bought the benchmark "{{ benchmark.name }}" (Isin: {{ benchmark.isin }})
          instead of your stock picks you would have gained:
        </p>
        <p><label>Total profit: </label> {{ benchmark.buyInstead.totalProfit }}</p>
        <p><label>Total ROI: </label> {{ benchmark.buyInstead.totalRoi }}</p>
        <p><label>Annual ROI: </label> {{ benchmark.buyInstead.totalRoiAnnual }}</p>

        <p>
          If you would have bought the benchmark "{{ benchmark.name }}" (Isin: {{ benchmark.isin }})
          every month with a rate of {{ benchmark.buyPlan.rate }} you would have gained:
        </p>
        <p><label>Total profit: </label> {{ benchmark.buyPlan.totalProfit }}</p>
        <p><label>Total ROI: </label> {{ benchmark.buyPlan.totalRoi }}</p>
        <p><label>Annual ROI: </label> {{ benchmark.buyPlan.totalRoiAnnual }}</p>
      </CCardBody>
    </CCard>
  </div>
</template>
